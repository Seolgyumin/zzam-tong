...
네이버 스마트스토어를 비롯한 온라인 쇼핑몰 상품 링크들을 7분마다 자동으로 확인해서,
품절이었던 상품이 재입고되면 Gmail로 알림 메일을 보내주는 자동화입니다.

GitHub Actions로 동작하기 때문에 내 컴퓨터나 휴대폰이 꺼져 있어도
GitHub 서버에서 계속 실행됩니다. 실제 헤드리스 크로미움 브라우저(Playwright)로
페이지에 접속해서 확인하기 때문에, 단순 요청만으로는 확인이 어려운 사이트도
어느 정도 대응할 수 있습니다.
...
`links.txt` 파일을 열어서 상품 링크를 한 줄에 하나씩 추가하고 저장(커밋)하세요.
GitHub 웹사이트에서 직접 파일을 열어 연필(✏️) 아이콘으로 편집한 뒤 "Commit changes"만 누르면 됩니다.

```
https://smartstore.naver.com/샵이름/products/1234567890
https://www.pokemonstore.co.kr/pages/product/product-detail.html?productNo=136570884
```

- 네이버 스마트스토어 링크뿐 아니라, "구매하기"/"품절" 같은 구매 버튼이 있는
  일반적인 쇼핑몰 상품 페이지도 함께 등록할 수 있습니다.
...
1. `scripts/check_stock.py` 가 헤드리스 크로미움 브라우저(Playwright)로 `links.txt`의
   모든 링크에 실제로 접속해서 재고 상태를 확인합니다. 판별은 아래 순서로 시도합니다.
   1. 네이버 상품 페이지에 내장된 데이터(JSON, `__NEXT_DATA__`)를 우선 분석
   2. 실패하면 "구매하기"/"품절"/"구매불가" 같은 구매 버튼의 텍스트와 활성화 여부로 판별
      (일반 쇼핑몰 대응)
   3. 그래도 실패하면 페이지 전체에서 "품절"/"재입고 알림" 같은 문구가 있는지로 최종 확인
...
5. 매 실행마다 브라우저가 접속할 사이트를 사람처럼 확인하는 과정이 있어서,
   단순 요청 방식보다 실행 시간이 조금 더 걸립니다(대략 1~2분 내외).

## 문제가 생기면

- 특정 상품만 재입고를 못 잡아낸다면, 해당 사이트의 페이지 구조가 바뀌었을 가능성이 있습니다.
  Actions 로그의 오류 메시지를 알려주시면 스크립트를 보완할 수 있습니다.
- 메일이 전혀 오지 않는다면 먼저 `GMAIL_ADDRESS`/`GMAIL_APP_PASSWORD` 시크릿이
  정확히 등록되어 있는지 확인해주세요.
- 일부 사이트(예: 네이버 공식 브랜드스토어 `brand.naver.com`)는 GitHub Actions 서버의
  IP 대역 자체를 차단하고 있어서, 브라우저로 접속해도 `HTTP 429` 오류가 계속 발생할 수
  있습니다. 이 경우 헤더나 로직을 아무리 보강해도 해결되지 않을 수 있으니, 가능하면
  일반 `smartstore.naver.com` 링크나 다른 쇼핑몰 링크로 대체해보시는 걸 권장합니다.
